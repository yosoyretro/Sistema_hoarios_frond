...
const Rol = () => {
    ...
    return(
        <>
        <Spin spinning={loading} tip="Cargando...">{/*MODAL DE CREAR PARALELO*/}<Modal title="Crear Rol" footer={null} open={modalOpen} onCancel={()=>{setModalOpen(false)}} onClose={()=>{setModalOpen(false)}} okText="Crear Rol">
            ...
            <Row>
                <Form form={form} onFinish={onFinish} layout="vertical" autoComplete="on" align="left">
                    <Row>
                        <Form.Item name="numero_paralelo" rules={[{required:true, message:"El rol es requerido"}]} label="ingrese el Rol">
                            <Input style={{ width:450}}/>
                        </Form.Item>
                    </Row>

                    <Row>
                        <Button htmlType="submit" style={{width:"100%"}} type="primary" icon={<SaveOutlined/>}>Crear Rol</Button>
                    </Row>
                </Form>
            </Row>
        </Modal>
        {/*MODAL DE EDITAR PARALELO*/}
        <Modal key={formKeyRef.current} title="Editar Rol" footer={null} open={modalOpenParalelo} onCancel={()=>{cerrarModalEdit()}} onClose={()=>{cerrarModalEdit()}}>
            <Row align="left" style={{marginLeft:"10px"}}>
                <h2><FileAddOutlined>Edita Rol</FileAddOutlined></h2>
            </Row>

            <Row>
                <Form initialValues={dataParaleloEdit} onFinish={editParalelo} layout="vertical" autoComplete="on" align="left">
                    <Row>
                        <Form.Item name="numero" rules={[{required:true,message: "El rol es requerido"}]} label="Edite el rol">
                            <Input style={{ width:450}}></Input>
                        </Form.Item>
                    </Row>
                    <Row>
                        <Button htmlType="submit" style={{width:"100%"}} type="primary" icon={<SaveOutlined/>}>Editar Rol</Button>
                    </Row>
                </Form>
            </Row>
        </Modal>
        ...
        <Row style={{margin:"5px"}}>
            <Breadcrumb separator=">" items={[{ title:"DashBoard"}, {title:"Rol"}]}/>
        </Row>
        <Space direction="vertical" size={20} style={{width:"100%"}}>
            <Row align="left" layout>
                <Space size="small" style={{ margin:"4px"}}>
                    <Col>
                        <Button style={{ color: "green", border: "1px solid green"}} icon={<EditOutlined/>} onClick={()=>{setModalOpen(true)}}>Crear Rol</Button>
                    </Col>
                    <Col>
                ...
                dataSource={dataParalelo}>
                    ...
                    <Column title = "Acciones" fixed="right" width={75} dataIndex="acciones" align="center" render={(Paralelo, record) => (
                        <Space size="small">
                            ...
                            <Col>
                                <Popconfirm okText="Si, realizar" title="Confirmar acción" description="¿Deseas realizar la eliminación de este registro? Al borrar este registro todos los usuarios que tengan el Rol de este registro quedarán afectados." onConfirm={() => {deleteParalelo(record)}} icon={<QuestionCircleOutlined style={{ color: 'red'}}/>}>
